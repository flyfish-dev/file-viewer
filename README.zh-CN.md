...
<p align="center">
  ...
  <img alt="Supported formats" src="https://img.shields.io/badge/formats-273-31a66f" />
  <img alt="Modular architecture" src="https://img.shields.io/badge/architecture-modular%20renderers-278cff" />
  <img alt="Ecosystem packages" src="https://img.shields.io/badge/npm%20targets-89-0f766e" />
</p>
...
当前内置 273 个已注册扩展名（224 个稳定、49 个实验）和 46 条预览链路。Office、PDF、OFD、Typst、CAD、STEP、XMind、压缩包、邮件、绘图、音视频、代码、Adobe 设计文件、字体、结构化数据、DICOM、数字签名容器和显式二进制检查器共享文件源、生命周期、搜索、缩放、打印、导出和下载契约；重型 Worker、WASM、字体与 vendor 资产保持按需加载，并可完全托管在自己的网络中。
...
- **矩阵可核验。** 273 个已注册扩展名（224 个稳定、49 个显式按需实验项）映射到 46 条预览链路，覆盖办公、工程、设计、数据、音视频、代码、DICOM、数字签名和二进制检查附件。
...
[`ecosystem/format-catalog.json`](ecosystem/format-catalog.json) 是唯一格式事实源：当前源码注册 273 个不重复扩展名和 46 条预览链路，其中 224 个稳定、49 个实验。下表按用户可理解的文件家族分组，并完整列出所有已注册扩展名。
...
当前版本以 npm registry 的 `latest` dist-tag 为准，本仓库共发布 89 个 npm 目标：81 个标准组件/完整 full 包/核心/renderer/preset/工程插件包 + 8 个历史兼容 alias；独立版本的 renderer 依赖不计入该数字。新项目建议优先使用 `@file-viewer/*` 标准包名；旧项目继续使用 `@flyfish-group/*` 或 `file-viewer3` 时也会拿到同版本能力。
...
生态当前维护 89 个 npm 发布目标（81 个标准包 + 8 个历史兼容包）；格式目录声明 46 条预览链路、273 个扩展名（已注册），其中 224 个稳定、49 个实验。格式说明见官方文档: https://doc.file-viewer.app/guide/formats
