...
**88 npm targets, 266 registered extensions, and 45 preview pipelines** mean you can start with the file problem you have today instead of assembling a different viewer for every attachment type.
...
Looking for one exact suffix? Search the maintained [266-extension format matrix](https://doc.file-viewer.app/guide/formats), which records the renderer, support level, evidence status, and deployment requirements for each pipeline.
