...
export const sampleGroupsZh: DemoSampleGroup[] = [
  ...
  {
    title: '3D 模型和地理数据',
    description: 'GLTF / STEP / IFC / OBJ / STL / GeoJSON / KML / GPX',
    family: 'model',
    items: [
      { name: 'GLTF', url: '/example/model.gltf' },
      { name: 'STEP 工程模型', url: '/example/model.step' },
      { name: 'IFC4 建筑模型（buildingSMART）', url: '/example/buildingSMART-ifc4-building.ifc' },
      { name: 'IFC4.3 建筑模型（buildingSMART）', url: '/example/buildingSMART-ifc43-building.ifc' },
  ...
  {
    title: '二进制检查器',
    description: 'HEX / PE / ELF / Mach-O / Java / 文件签名',
    family: 'binary',
    items: [
      { name: '原始字节（128 KiB）', url: '/example/binary-raw.bin' },
      { name: 'PNG 文件签名结构', url: '/example/binary-png-signature.hex' },
      { name: 'WebAssembly 文件签名结构', url: '/example/binary-wasm-signature.hex' },
      { name: 'ELF 可执行结构', url: '/example/binary-elf.elf' },
      { name: 'PE 可执行结构（EXE）', url: '/example/binary-pe.exe' },
      { name: 'PE 动态库结构（DLL）', url: '/example/binary-pe.dll' },
      { name: 'Mach-O 可执行结构', url: '/example/binary-macho.macho' },
      { name: 'ZIP 文件签名结构', url: '/example/binary-zip-signature.hex' },
      { name: 'Java Class 结构', url: '/example/binary-java.class' }
    ]
  },
  {
    title: 'Adobe 设计',
    ...
    items: [
      ...
      { name: 'IDML 双页 CPU WASM 渲染', url: '/example/indesign-layout.idml' },
      ...
      { name: 'INX 960 栅格旧版交换文件', url: '/example/indesign-grid-legacy.inx' },
      { name: 'FLA 现代压缩 XFL 首帧与时间轴（项目生成）', url: '/example/animate-modern-xfl.fla' },
      { name: 'XD 多画板嵌入预览与结构', url: '/example/adobe-xd-grids.xd' },
      { name: 'INDD 2025 严格 XMP 预览', url: '/example/indesign-2025.indd' },
      { name: 'INDT 2025 严格 XMP 预览', url: '/example/indesign-template-2025.indt' },
      { name: 'ASE 分组色板（CC0）', url: '/example/adobe-palette.ase' },
      { name: 'ACO 多语言色板', url: '/example/adobe-colors.aco' },
...
const englishGroupCopy: Array<Pick<DemoSampleGroup, 'title' | 'description'>> = [
  ...
  { title: '3D Models & Geospatial Data', description: 'GLTF / STEP / IFC / OBJ / STL / GeoJSON / KML / GPX' },
  ...
  { title: 'Binary Inspector', description: 'Hex / PE / ELF / Mach-O / Java / File signatures' },
...
const englishSampleNameMap: Record<string, string> = {
  ...
  '/example/buildingSMART-ifc4-building.ifc': 'IFC4 building model (buildingSMART)',
  '/example/buildingSMART-ifc43-building.ifc': 'IFC4.3 building model (buildingSMART)',
  ...
  '/example/binary-raw.bin': 'Raw bytes (128 KiB)',
  '/example/binary-png-signature.hex': 'PNG file-signature structure',
  '/example/binary-wasm-signature.hex': 'WebAssembly file-signature structure',
  '/example/binary-elf.elf': 'ELF executable structure',
  '/example/binary-pe.exe': 'PE executable structure (EXE)',
  '/example/binary-pe.dll': 'PE dynamic-library structure (DLL)',
  '/example/binary-macho.macho': 'Mach-O executable structure',
  '/example/binary-zip-signature.hex': 'ZIP file-signature structure',
  '/example/binary-java.class': 'Java Class structure',
  ...
  '/example/indesign-layout.idml': 'IDML two-page CPU WASM render',
  ...
  '/example/indesign-grid-legacy.inx': 'Legacy INX 960 grid document',
  '/example/animate-modern-xfl.fla': 'Modern compressed XFL-based FLA first frame and timeline (project-generated)',
  '/example/adobe-xd-grids.xd': 'XD multi-artboard embedded preview and structure',
  '/example/indesign-2025.indd': 'INDD 2025 verified XMP preview',
  '/example/indesign-template-2025.indt': 'INDT 2025 verified XMP preview',
  '/example/adobe-palette.ase': 'ASE grouped color palette (CC0)',
  '/example/adobe-colors.aco': 'ACO multilingual color palette',
...
const japaneseGroupCopy: Array<Pick<DemoSampleGroup, 'title' | 'description'>> = [
  ...
  { title: '3D モデルと地理空間データ', description: 'メッシュ、CAD カーネル、IFC、GeoJSON' },
  ...
  { title: 'バイナリインスペクター', description: 'HEX / PE / ELF / Mach-O / Java / ファイル署名' },
...
const japaneseSampleNameMap: Record<string, string> = {
  ...
  '/example/buildingSMART-ifc4-building.ifc': 'IFC4 建築モデル（buildingSMART）',
  '/example/buildingSMART-ifc43-building.ifc': 'IFC4.3 建築モデル（buildingSMART）',
  ...
  '/example/binary-raw.bin': '生バイト列（128 KiB）',
  '/example/binary-png-signature.hex': 'PNG ファイル署名構造',
  '/example/binary-wasm-signature.hex': 'WebAssembly ファイル署名構造',
  '/example/binary-elf.elf': 'ELF 実行ファイル構造',
  '/example/binary-pe.exe': 'PE 実行ファイル構造（EXE）',
  '/example/binary-pe.dll': 'PE 動的ライブラリ構造（DLL）',
  '/example/binary-macho.macho': 'Mach-O 実行ファイル構造',
  '/example/binary-zip-signature.hex': 'ZIP ファイル署名構造',
  '/example/binary-java.class': 'Java Class 構造',
  ...
  '/example/indesign-layout.idml': 'IDML 2 ページ CPU WASM レンダリング',
  ...
  '/example/indesign-grid-legacy.inx': '従来 INX 960 グリッド文書',
  '/example/animate-modern-xfl.fla': '最新の圧縮 XFL ベース FLA の先頭フレームとタイムライン（プロジェクト生成）',
  '/example/adobe-xd-grids.xd': 'XD マルチアートボード埋め込みプレビューと構造',
  '/example/indesign-2025.indd': 'INDD 2025 検証済み XMP プレビュー',
  '/example/indesign-template-2025.indt': 'INDT 2025 検証済み XMP プレビュー',
  '/example/adobe-palette.ase': 'ASE グループカラーパレット（CC0）',
  '/example/adobe-colors.aco': 'ACO 多言語カラーパレット',
