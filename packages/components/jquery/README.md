...
标准 jQuery native 组件包。它通过本包本地 controller 调用 `@file-viewer/core` 与 core browser engine 挂载完整预览器，只提供轻量的 `$(el).fileViewer()` 接入层。
...
如果页面中已有 `window.jQuery` 或 `window.$`，包会自动注册 `$.fn.fileViewer`。纯 script 标签场景建议优先使用构建工具或自托管 ESM/IIFE 入口；如需自托管 PDF、CAD、压缩包等 Worker/WASM/vendor 资源，运行 `file-viewer-copy-assets` 并在 options 中配置对应资源路径。

## 能力范围

`@file-viewer/jquery` 与其他标准组件包共享同一套 core 接口和 renderer/preset 装配模型；完整能力可通过按需 renderer 或 `@file-viewer/preset-all` 覆盖 PDF、Word、Excel、PPT、OFD、CAD/DWG/DXF/DWF、EPUB/UMD、压缩包、邮件、Markdown、代码高亮、图片、音频、视频、3D 模型、地理数据和结构化数据资产等预览链路。完整格式矩阵和参数说明见官方文档: https://doc.file-viewer.app/guide/formats
...
```bash
npm i @file-viewer/jquery @file-viewer/preset-office
```
...
```bash
npm i @file-viewer/jquery @file-viewer/preset-all
```
