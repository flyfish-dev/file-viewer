...
标准 Svelte native 组件包，提供 Svelte 组件和 action 两种接入方式。组件和 action 都通过本包本地 controller 调用 `@file-viewer/core` 与 core browser engine 挂载完整预览器；SSR 环境下只在浏览器端挂载。

```bash
npm install @file-viewer/svelte
```

包根入口的默认导出固定为 Svelte 组件。Action 与底层 controller helper 分别从 `@file-viewer/svelte/action`、`@file-viewer/svelte/controller` 导入，避免组件和 action 的默认导出语义冲突。
...
组件实例和 action 都支持 `update`、`reload` 和 `destroy` 等底层 controller 行为。

## 能力范围

`@file-viewer/svelte` 与其他标准组件包共享同一套 core 接口和 renderer/preset 装配模型；完整能力可通过按需 renderer 或 `@file-viewer/preset-all` 覆盖 PDF、Word、Excel、PPT、OFD、CAD/DWG/DXF/DWF、EPUB/UMD、压缩包、邮件、Markdown、代码高亮、图片、音频、视频、3D 模型、地理数据和结构化数据资产等预览链路。完整格式矩阵和参数说明见官方文档: https://doc.file-viewer.app/guide/formats
...
共享格式目录当前注册 245 个扩展名（222 个稳定、23 个实验），映射到 35 条预览链路。实验格式不计入稳定支持数量；完整能力通过 renderer / preset 按需装配。
