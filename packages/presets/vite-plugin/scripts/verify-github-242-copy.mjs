...
try {
  for (const mode of ['missing-font', 'valid-font', 'broken-font', 'missing-worker']) {
    ...
    await file('node_modules/@file-viewer/renderer-pdf/package.json', JSON.stringify({ name: '@file-viewer/renderer-pdf', version: '1.0.0', main: 'index.js', dependencies: { 'pdfjs-dist': '1.0.0' } }))
    ...
    if (mode === 'valid-font' || mode === 'broken-font') {
      ...
      if (mode === 'valid-font') await file('node_modules/@fontsource-variable/noto-sans-sc/LICENSE', 'synthetic-license-fixture')
    }
    ...
    if (mode === 'broken-font' || mode === 'missing-worker') {
      await assert.rejects(plugin.closeBundle(), mode === 'broken-font' ? /pdf-cjk-font-fallback/ : /pdf-worker/)
    } else {
      await plugin.closeBundle()
      assert.equal(await readFile(join(root, 'dist/assets/vendor/pdf/pdf.worker.mjs'), 'utf8'), 'fixture:legacy/build/pdf.worker.mjs')
      if (mode === 'missing-font') assert.ok(warnings.some(value => value.includes('pdf-cjk-font-fallback')), 'Missing optional font must produce an actionable warning')
      else {
