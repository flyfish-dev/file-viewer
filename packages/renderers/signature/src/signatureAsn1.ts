...
const toUint8Array = (value: ArrayBuffer | Uint8Array) => value instanceof Uint8Array
  ? value
  : new Uint8Array(value);

const bytesToHex = (bytes: Uint8Array, separator = '') => Array.from(
  bytes,
...
const formatOid = (oid?: string) => oid
  ? OID_LABELS[oid] || oid
  : 'Unknown';

const assertBounds: (condition: unknown, message: string) => asserts condition = (condition, message) => {
  if (!condition) {
...
const parseRoot = (bytes: Uint8Array) => {
...
const isTag = (node: Asn1Node | undefined, tagClass: number, tagNumber: number) => Boolean(
  node && node.tagClass === tagClass && node.tagNumber === tagNumber
);

const contentBytes = (bytes: Uint8Array, node: Asn1Node) => bytes.subarray(
  node.contentStart,
  node.contentEnd
);
...
const integerHex = (bytes: Uint8Array, node: Asn1Node) => bytesToHex(positiveIntegerBytes(bytes, node));
...
const parseTimeString = (value: string, generalized: boolean) => {
  ...
  let millis = fraction ? Number(`0.${fraction}`) * 1000 : 0;
...
const collectOctetString = (bytes: Uint8Array, node: Asn1Node): Uint8Array => {
  ...
  const parts = node.children
    ...
    .map(child => collectOctetString(bytes, child));
  if (!parts.length) {
...
const parseExtensions = (
  ...
  for (const extension of extensions) {
    ...
    const valueNode = [...extension.children].reverse().find(child => isTag(child, UNIVERSAL, TAG.octetString));
...
const parseCertificate = (
  ...
  const validity = validityNode.children.filter(child => (
    isTag(child, UNIVERSAL, TAG.utcTime) || isTag(child, UNIVERSAL, TAG.generalizedTime)
  ));
...
const parseSigner = (
  ...
  const signedAttributesNode = isTag(children[cursor], CONTEXT, 0)
    ? children[cursor++]
    : undefined;
  const signatureAlgorithmNode = children[cursor++];
  const signatureNode = children[cursor++];
  const unsignedAttributesNode = isTag(children[cursor], CONTEXT, 1)
    ? children[cursor]
    : undefined;
  ...
  for (const child of signedAttributesNode?.children || []) {
    ...
    } else if (attribute.oid === OID.messageDigestAttribute && isTag(value, UNIVERSAL, TAG.octetString)) {
...
const parseCmsContentInfo = (bytes: Uint8Array, root: Asn1Node): CmsRecord => {
  ...
  const embeddedContent = eContentNode ? collectOctetString(bytes, eContentNode) : undefined;
  ...
  const signers = (signerInfosNode?.children || [])
    .map((child, index) => parseSigner(bytes, child, index))
...
const verifySigner = async (
  ...
  if (content && summary.messageDigest) {
    ...
    if (digest) {
      summary.digestMatches = equalBytes(digest, Uint8Array.from(
...
const enrichCertificateFingerprints = async (certificates: CertificateRecord[], bytes: Uint8Array) => {
  const subtle = subtleCrypto();
  if (!subtle) {
    return;
  }
...
const parseAccuracy = (bytes: Uint8Array, node: Asn1Node) => {
  ...
  for (const child of node.children) {
    if (isTag(child, UNIVERSAL, TAG.integer)) {
      ...
      accuracy.millis = value ? integerNumber(bytes, value) : Number.parseInt(bytesToHex(contentBytes(bytes, child)), 16);
    } else if (isTag(child, CONTEXT, 1)) {
      const value = child.children[0];
      accuracy.micros = value ? integerNumber(bytes, value) : Number.parseInt(bytesToHex(contentBytes(bytes, child)), 16);
    }
  }
  return accuracy;
};
...
const parseTstInfo = async (
  ...
  const summary: TimestampInfoSummary = {
    ...
    policyOid: policyNode && isTag(policyNode, UNIVERSAL, TAG.oid) ? decodeOid(bytes, policyNode) : undefined,
...
const baseWarnings = () => [
  ...
  'Phase 1 covers CMS/PKCS#7, CAdES attributes, RFC 3161, and RFC 5544 TimeStampedData. OpenPGP, ASiC, evidence records, PAdES, XAdES, and JAdES are out of scope.',
  'No private keys, passphrases, trust stores, network lookups, or LGPL OpenPGP source are used or bundled.',
];
...
const inspectCmsNode = async (
  ...
  const cms = parseCmsContentInfo(bytes, root);
  ...
  const timestamp = cms.signedContentTypeOid === OID.tstInfo && cms.embeddedContent
    ? await parseTstInfo(cms.embeddedContent, originalContent)
    : undefined;
  return {
    kind,
...
const decodePemIfNeeded = (input: Uint8Array) => {
  ...
  const match = text.match(/-----BEGIN (?:PKCS7|CMS)-----([\s\S]*?)-----END (?:PKCS7|CMS)-----/);
  assertBounds(match, 'Unsupported PEM cryptographic container. Expected PKCS7 or CMS armor.');
  const compact = match[1]!.replace(/\s+/g, '');
  assertBounds(compact.length > 0 && compact.length % 4 === 0, 'Malformed PEM base64 payload.');
  const binary = typeof atob === 'function'
    ? atob(compact)
    : (() => { throw new Error('PEM decoding requires a browser with atob support.'); })();
  const output = new Uint8Array(binary.length);
  for (let index = 0; index < binary.length; index += 1) output[index] = binary.charCodeAt(index);
  return output;
};

const findCmsContentInfoNode = (bytes: Uint8Array, node: Asn1Node, depth = 0): Asn1Node | undefined => {
  if (depth > 12) return undefined;
  if (isTag(node, UNIVERSAL, TAG.sequence)) {
    const first = node.children[0];
    if (first && isTag(first, UNIVERSAL, TAG.oid) && decodeOid(bytes, first) === OID.signedData) return node;
  }
  for (const child of node.children) {
...
const parseTimestampedData = async (
  ...
  if (isTag(fields[cursor], UNIVERSAL, TAG.ia5String)) dataUri = decodeString(bytes, fields[cursor++]!);
  ...
  if (metadataNode) {
    ...
    if (metadata[0] && isTag(metadata[0], UNIVERSAL, TAG.boolean)) hashProtected = decodeBoolean(bytes, metadata[0]);
    for (const child of metadata.slice(1)) {
      if (!filename && (isTag(child, UNIVERSAL, TAG.utf8String) || isTag(child, UNIVERSAL, TAG.printableString))) filename = decodeString(bytes, child);
      else if (!mediaType && isTag(child, UNIVERSAL, TAG.ia5String)) mediaType = decodeString(bytes, child);
    }
  }
  const embeddedNode = fields[cursor] && isTag(fields[cursor], UNIVERSAL, TAG.octetString) ? fields[cursor++] : undefined;
  const embeddedContent = embeddedNode ? collectOctetString(bytes, embeddedNode) : undefined;
  const evidenceNode = fields[cursor];
  const cmsNode = evidenceNode ? findCmsContentInfoNode(bytes, evidenceNode) : undefined;
  const evidenceInspection = cmsNode ? await inspectCmsNode(bytes, cmsNode, embeddedContent || originalContent) : undefined;
  ...
  return {
    ...
    timestampedData: { dataUri, filename, mediaType, hashProtected, temporalEvidenceEntries: evidenceEntries },
    warnings: [
      ...baseWarnings(),
      ...(dataUri ? ['External RFC 5544 dataUri values are displayed as untrusted metadata and are never fetched automatically.'] : []),
    ],
  };
};
...
export const inspectSignatureContainer = async (
  ...
  if (first && isTag(first, UNIVERSAL, TAG.oid)) {
    ...
    if (contentTypeOid === OID.timestampedData) {
      return parseTimestampedData(bytes, root, originalContent);
    }
    return inspectCmsNode(bytes, root, originalContent);
  }
  ...
  if (looksLikeTimestampRequest(root)) {
    ...
    return {
      ...
      sourceSize: bytes.byteLength,
      digestAlgorithms: timestamp.messageImprintAlgorithm ? [timestamp.messageImprintAlgorithm] : [],
      detached: true,
  ...
  if (looksLikeTimestampResponse(root)) {
    ...
    if (tokenNode) {
      const tokenInspection = await inspectCmsNode(bytes, tokenNode, originalContent);
      return {
        ...tokenInspection,
