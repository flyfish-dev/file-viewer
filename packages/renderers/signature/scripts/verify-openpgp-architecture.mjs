...
assert.match(client, /\.\.\/signature\.worker\.js/, 'The Worker must be created from the emitted optional renderer asset.');

const rendererIndex = await read('src/index.ts');
for (const extension of ['asc', 'sig', 'pgp', 'gpg']) {
  assert.match(rendererIndex, new RegExp(`['\"]${extension}['\"]`), `${extension} must be registered by the optional signature renderer.`);
}

const protocol = await read('src/workerProtocol.ts');
for (const operation of ['classify', 'inspect', 'verify-detached']) {
  assert.match(protocol, new RegExp(`type:\\s*['\"]${operation}['\"]`), `Worker protocol must include ${operation}.`);
}
for (const forbidden of ['decrypt', 'sign-message', 'generate-key', 'unlock-private-key']) {
  assert.doesNotMatch(protocol, new RegExp(`type:\\s*['\"]${forbidden}['\"]`), `Sensitive operation ${forbidden} must not be exposed.`);
}

const rustLib = await read('rust/src/lib.rs');
for (const exported of ['classify_openpgp', 'inspect_openpgp', 'verify_detached_signature']) {
  assert.match(rustLib, new RegExp(`pub fn ${exported}\\b`), `Rust wrapper must export ${exported}.`);
}
assert.doesNotMatch(rustLib, /pub fn (?:decrypt|sign|generate_key|unlock_private)/, 'Rust wrapper must not expose private-key operations.');
...
async function collectSourceFiles(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    else if (['.ts', '.js', '.mjs', '.rs', '.json', '.toml'].includes(extname(entry.name))) result.push(path);
  }
  return result;
}
