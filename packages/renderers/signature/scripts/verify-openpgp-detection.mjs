...
assert.equal(
  isProbablyOpenPgp(enc.encode('plain text'), 'document.sig'),
  true,
  'Known OpenPGP extensions are routing hints.'
)
assert.equal(
  isProbablyOpenPgp(enc.encode('plain text'), 'document.txt'),
  false,
  'Unrelated plain text must not be classified as OpenPGP.'
)
assert.equal(
  hasPlausibleOpenPgpPacketHeader(Uint8Array.from([0xc8, 0x00])),
  true,
  'Plausible binary packet header must be recognized.'
)
