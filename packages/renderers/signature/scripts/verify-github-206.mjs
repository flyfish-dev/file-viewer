...
import { DEFAULT_SIGNATURE_ASN1_LIMITS, inspectSignatureContainer } from '../dist/signatureAsn1.js'

const here = dirname(fileURLToPath(import.meta.url))
const fixtures = resolve(here, '../test/fixtures/github-206-contributed')
const read = (name) => readFile(resolve(fixtures, name))
const inspect = async (name, original) =>
  inspectSignatureContainer(
    await read(name),
    original
      ? { originalContent: await read(original), sourceFilename: name }
      : { sourceFilename: name }
  )
...
assert.equal(
  multi.signers.every((signer) => signer.cryptographicSignatureValid === true),
  true
)
...
await assert.rejects(
  async () =>
    inspectSignatureContainer(await read('cms/hello-multi-signer.p7m'), {
      limits: { maxSigners: 1 }
    }),
  /signer count exceeds 1/i
)
await assert.rejects(
  async () =>
    inspectSignatureContainer(await read('cms/hello-multi-signer.p7m'), {
      limits: { maxAttributesPerSigner: 1 }
    }),
  /attribute count exceeds 1/i
)
...
const adversarialPem = Buffer.from(
  `-----BEGIN CMS-----\n${'-----BEGIN CMS-----a'.repeat(50_000)}\n-----END CMS-----\n`,
  'ascii'
)
await assert.rejects(
  () => inspectSignatureContainer(adversarialPem),
  /malformed PEM base64 payload/i
)
await assert.rejects(
  () => inspectSignatureContainer(Buffer.from(
    '-----BEGIN CMS-----\nAAAA\n-----END PKCS7-----\n',
    'ascii'
  )),
  /missing its matching footer/i
)
await assert.rejects(
  () => inspectSignatureContainer(Buffer.from(
    '-----BEGIN CMS-----\nAA=A\n-----END CMS-----\n',
  ...
  /malformed PEM base64 padding/i
)
...
await assert.rejects(
  async () =>
    inspectSignatureContainer(await read('cms/certificate-chain.p7b'), {
      limits: { maxCertificates: 1 }
    }),
  /certificate count exceeds 1/i
)
...
const tsdExternal = await inspect(
  'timestamps/invoice-external-content.tsd',
  'originals/invoice.pdf'
)
...
await assert.rejects(
  async () =>
    inspectSignatureContainer(await read('cms/invoice-encapsulated.pdf.p7m'), {
      limits: { maxEmbeddedContentBytes: 16 }
    }),
  /embedded content exceeds the 16-byte boundary/i
)
await assert.rejects(
  async () =>
    inspectSignatureContainer(await read('cms/invoice-detached.pdf.p7s'), {
...
await assert.rejects(
  async () =>
    inspectSignatureContainer(await read('cms/invoice-encapsulated.pdf.p7m'), {
      limits: { maxInputBytes: DEFAULT_SIGNATURE_ASN1_LIMITS.maxInputBytes + 1 }
    }),
  /maxInputBytes must be an integer between/i
)
...
assert.equal(
  dependencyNames.some((name) => /openpgp|gnupg|gpg/i.test(name)),
  false,
  'The renderer must not introduce OpenPGP.js, GnuPG, GPG, or other LGPL OpenPGP dependencies.'
)
...
for (const path of await allFiles(fixtures)) {
  ...
  assert.equal(
    /-----BEGIN (?:RSA |EC |OPENSSH |ENCRYPTED )?PRIVATE KEY-----/.test(data.toString('utf8')),
    false,
    `Private key marker found in ${path}`
  )
}
