...
<p align="center">
  ...
  <img alt="Supported formats" src="https://img.shields.io/badge/formats-265-31a66f" />
...
The current source catalog maps 265 registered extensions (223 stable, 42 experimental) to 44 preview pipelines. Office, PDF, OFD, Typst, CAD, STEP, XMind, archives, email, diagrams, media, source code, Adobe design files, fonts, structured data, DICOM, and digital-signature containers share the same source, lifecycle, search, zoom, print, export, and download contracts. Heavy Worker, WASM, font, and vendor assets stay lazy and can be hosted entirely on your network.
...
- **Use the checked matrix.** 265 registered extensions map to 44 preview pipelines across office, engineering, design, data, media, code, DICOM, and digital-signature attachments; 223 are stable and 42 are explicit experimental opt-ins.
...
`*-full` means the published `preset-all` compatibility matrix and its same-version Worker, WASM, font, and vendor payload are included—do not install or pass another preset. Later specialist renderers such as Adobe design, DICOM, and digital-signature containers remain explicit additions, so an unchanged Full dependency does not grow without bound. Vite publishes packaged assets automatically; other bundlers self-host them with the same-version CLI included by the Full package.
...
The `web-full` CDN IIFE only loads the Custom Element, controller, and lazy full preset up front. Heavy PDF, Word, Excel, binary PPT, PPTX, CAD, Typst, archive, and similar renderer bundles are fetched asynchronously from `dist/renderers/*.iife.js` only when the active file type needs them. Deploy the complete `dist/`: it contains the verified `@file-viewer/ppt@0.3.3` ESM, Worker, WASM, CJK font, and frame-cache runtime under `vendor/ppt/`, together with the other version-aligned assets. Binary `.ppt` therefore works without runtime URL configuration. `pptModuleUrl`, `pptWorkerUrl`, `pptWasmUrl`, and `pptFontUrl` are advanced overrides for non-standard asset routing.
...
Every full package points Archive, PDF, DOCX, Excel, binary PPT, PPTX, CAD, Typst, Draw.io, SQLite, and the remaining runtime assets to `file-viewer/` under the deployment base by default (`/file-viewer/` at the origin root). After Vite publishes the included assets or the bundled copy CLI writes `./public/file-viewer`, those URLs do not need to be configured. The verified `@file-viewer/ppt@0.3.3` runtime is included under `vendor/ppt/`; its own LICENSE and NOTICE remain intact.
...
- `@file-viewer/renderer-*`: independent PDF, Word, PPT/PPTX, CAD, Typst, Archive, Drawing, Data, EDA, and other rendering capabilities.
...
The source of truth is [`ecosystem/format-catalog.json`](ecosystem/format-catalog.json): the current source registers 265 unique extensions in 44 preview pipelines, with 223 stable and 42 experimental. The table groups those pipelines into readable product families and lists every registered extension.
...
| Adobe design files             | `psd`, `psb`, `pdd`, `psdt`, `ai`, `ait`, `eps`, `ps`, `idml`, `icml`, `idms`, `inx`, `xd`, `indd`, `indt`, `fla`, `xfl`, `ase`, `aco`, `abr`, `csh`, `pat`, `grd`, `asl`                                                                                                                                                                   | Explicit `@file-viewer/renderer-design`: saved PSD/PSB pixels and layers, PDF-compatible plus native-PGF Illustrator artboards/layers/paths, IDML CPU-WASM pages, bounded InDesign/XD/Animate structure and previews, Photoshop resources, palettes, and PostScript WASM. Unsupported native semantics remain visibly bounded. | Design review, asset libraries, offline creative-file intake    |
| Fonts and structured data      | `ttf`, `otf`, `woff`, `woff2`, `sqlite`, `wasm`, `parquet`, `avro`, `webarchive`                                                                                                                                                                                                                                                                        | Standalone `@file-viewer/renderer-data` with FontFace previews, SQLite/Parquet/Avro/WASM structural summaries, and configurable SQLite WASM URL. It remains the lightweight compatibility fallback for PSD/AI/EPS when the explicit design renderer is not assembled.                                                                                         | Font review, local databases, binary package inspection         |
...
Adobe creative-file support is an explicit specialist package: install [`@file-viewer/renderer-design`](https://www.npmjs.com/package/@file-viewer/renderer-design) for PSD/PSB/PDD/PSDT, AI/AIT, EPS/PS, InDesign exchange and embedded previews, XD, modern FLA/XFL, palettes, and Photoshop resource presets. It is not silently included in the frozen `preset-all` / Full compatibility baseline.
...
The ecosystem currently maintains 86 npm targets (78 standard packages plus 8 historical aliases). The catalog declares 44 preview pipelines and 265 file extensions: 223 stable and 42 experimental. See https://doc.file-viewer.app/guide/formats
