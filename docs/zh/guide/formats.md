...
<div class="doc-shot">
  <img src="/_media/file-viewer-demo-v2.2.6-samples-zh.webp" alt="Flyfish Viewer v2.2.6 中文格式样例库，展示分组、文件名与格式专属图标" width="1440" height="900" loading="lazy" />
...
- EPUB 链路已拆为 `@file-viewer/renderer-epub` 独立包。v2.2.6 将阅读引擎构建为确定性的包内 vendor 资产，连同依赖许可 NOTICE 一起分发；只有命中 `.epub` 时才加载，不需要公网脚本，也不会把旧版 XML 解析依赖带进业务生产依赖树。
