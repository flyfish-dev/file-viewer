...
<p class="doc-lead">
  唯一格式目录当前注册 <strong>266 个扩展名</strong>，覆盖 <strong>45 条预览链路</strong>：其中 <strong>224 个稳定</strong>、<strong>42 个实验</strong>。
...
<div class="doc-shot">
  ...
  <p class="doc-caption">Demo 把 45 条预览链路的代表样例按类型分组；稳定格式必须有可再分发的真实文件 fixture 与浏览器断言，合成或改后缀样例不计入证据；实验项必须持续展示限制。</p>
</div>
...
| IFC / BIM（显式按需） | `ifc` | `@file-viewer/capability-ifc` + `@file-viewer/renderer-3d` + `@file-viewer/assets-ifc` | 浏览器本地 `web-ifc` 解析、Three.js 交互、元素选择、实体类型 / `Name` / `GlobalId` 与属性集检查；不进入 Engineering / Full 默认闭包 | BIM 附件快速审阅 |
...
- `ifc` 通过显式安装的 `@file-viewer/capability-ifc` + `@file-viewer/assets-ifc` 在浏览器本地解析并构建 Three.js BIM 场景，支持旋转/平移/缩放、适配、元素选择与属性检查；`3dm` 仍只展示签名识别和明确接入说明，后续沿 `rhino3dm` 路线独立维护。
