...
| Email | `postal-mime` 最新 npm 版本为 `3.0.0`，支持 Node、browser、Web Worker 和 serverless；3.0.0 修正了非标准 charset 标签回退、RFC 5322 头折叠展开、重复单值头取首个以及 to / cc / bcc 地址顺序，EML 头解析依赖这些修正，因此 renderer 直接跟随升级到 `^3.0.0`；`@kenjiuno/msgreader` 最新 npm 版本为 `1.28.0`，适合作为 MSG 读取层。 | 保持 `@file-viewer/renderer-email` 分别处理 EML/MBOX 与 MSG，正文沙箱化，附件继续复用统一预览器；`pnpm test:email-github-232` 把上述头解析行为锁成回归门禁。 |
...
| 3DM | `@file-viewer/renderer-3d` 与 `@file-viewer/geometry-engine` 保留签名识别和明确的接入说明，不虚标为成功预览 | 后续接入独立 `rhino3dm` / Three.js loader，并继续在独立几何包内演进 |
...
- [x] 使用 `pnpm verify:format-support` 校验 273 个注册扩展名（224 个稳定、49 个实验）和 46 条 renderer pipeline 口径一致。
...
- [x] 为 STEP / IGES / IFC / 3DM / BREP 建立独立 `@file-viewer/geometry-engine` 边界包，避免几何内核进入默认 core install path；STEP / STP、IGES / IGS、BREP 已完成离线 OCCT Worker/WASM 预览，IFC 已有显式 `@file-viewer/renderer-3d/ifc` 的本地 That Open / `web-ifc` 实验性路径，3DM 继续沿 `rhino3dm` 路线分层演进。
