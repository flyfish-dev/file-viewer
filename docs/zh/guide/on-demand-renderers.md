...
- [x] `@file-viewer/preset-all` 保持既有 221 个扩展名、32 条链路的兼容能力；当前新增的 Adobe 设计、DICOM 与数字签名/证据能力均为显式按需渲染器，不会静默进入既有 preset/full 依赖闭包。完整源码目录当前为 265 个扩展名（223 个稳定、42 个实验）和 44 条链路。
