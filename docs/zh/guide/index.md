...
<div class="doc-grid">
  ...
  <div class="doc-card">
    <h3>覆盖真实附件场景</h3>
    <p>唯一目录注册 273 个扩展名和 46 条预览链路，其中 224 个稳定、49 个实验；覆盖 Office、Apple 文稿、PDF/OFD、工程、归档、媒体、代码、数据、可选 DICOM、数字签名与二进制检查格式。</p>
  </div>
  <div class="doc-card">
    <h3>模块化架构更专业</h3>
...
<div class="doc-grid">
  ...
  <div class="doc-card">
    <h3>确认格式边界</h3>
    <p>支持格式页列出 273 个已注册扩展名、46 条渲染链路、稳定/实验等级和真实业务边界。</p>
  </div>
  <div class="doc-card">
    <h3>选择接入方式</h3>
...
- Excalidraw 默认使用 `roughjs` 只读 SVG，运行环境提供官方 ESM 模块时会优先尝试官方导出；draw.io 默认走不执行文档 HTML 的内置 SVG 预览，显式设置 `options.drawing.preferOfficial = true` 才在受限沙箱中加载单独分发的 diagrams.net 离线 viewer；Mermaid 走官方 `mermaid` SVG 渲染，PlantUML 默认离线源码预览，也可配置自托管 SVG 服务；这些绘图链路都支持拖拽平移和统一缩放。
