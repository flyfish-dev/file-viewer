...
| `text` | `toolbar: false` 可隐藏 renderer 内部元信息栏，`lineNumbers: true` 开启不干扰复制的行号。`wrapLongLines: true` 只做视觉换行，不改变源码字节，并同样作用于有界大文本视图。`prettyPrint: true` 会为支持的结构化文本按需加载 Prettier，将格式化结果作为显示副本；工具栏徽标与切换按钮明确区分格式化预览和原始源码。`prettyPrintMaxBytes` 仅限制格式化，默认继承 `virtualizeAboveBytes`（未配置时 512 KiB）；超限、语法错误或不支持的输入无错误回退，继续由既有普通/虚拟 renderer 展示。Markdown 默认保持排版阅读视图，仅在需要超大源码检查时设置 `markdownVirtualizeAboveBytes`。历史 `*-full` 的 script 标签 IIFE 资源不打包 Prettier，该路径下 `prettyPrint` 回退原始源码。 |
...
支持「页面预览」和「HTML 源码」切换，默认显示页面。传入 `options.text.htmlView: 'source'` 可以默认查看源码。页面预览保留内联样式和内嵌图片，在独立沙箱中展示静态内容，不执行脚本、提交表单、跳转外链或加载外部资源；它不是用于运行完整网站的浏览器。源码视图保留原文并支持现有的高亮、格式化和大文本虚拟化。
