...
`copyAssets` copies the offline payload that the installed packages provide. The PDF CJK fallback font ships in `@file-viewer/assets-standard` (also installed by `@file-viewer/preset-standard`); without it the build still succeeds and the plugin only warns that the font was skipped, leaving PDF rendering to embedded fonts.
