...
| --- | --- | --- |
| Formats | 221 registered extensions (221 stable, 0 experimental) across 32 preview pipelines for PDF/OFD, Office/iWork, CAD, Typst, archives, email, ebooks, diagrams, media, 3D, and data assets; enabled through `preset-lite`, `preset-office`, `preset-engineering`, or `preset-all` | Strengthens the Word, Excel, and PowerPoint deep end. It can replace the Word / Spreadsheet / Presentation capability in `preset-office` while PDF, OFD, CAD, Archive, and other formats continue to use open-source renderers |
