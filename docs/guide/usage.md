...
| `text.encoding` | Decoder for text, code, Markdown, HTML source, XML, and patch files: `auto` (default), `utf-8`, `utf-16le`, `utf-16be`, `gbk`, `gb18030`, `iso-8859-1`, `iso-8859-2`, `iso-8859-15`, `windows-1250`, `windows-1251`, or `windows-1252`. Auto mode checks a BOM, UTF-16 byte structure, and strict UTF-8 before using `text.fallbackEncoding`. Browsers decode the `iso-8859-1` label with the windows-1252 table, as the WHATWG Encoding Standard requires. |
| `text.fallbackEncoding` | Encoding that `auto` uses when the bytes are not valid UTF-8. Defaults to `gb18030`, which also covers GBK. Set `windows-1252` for Western European legacy files or `windows-1251` for Cyrillic ones. Single-byte text is structurally valid GB18030, so it cannot be detected automatically and otherwise renders as CJK characters such as `Saarbr點ken`. Download still returns the original bytes. |
