...
| `docx.reviewMode` | Text revisions in DOC and DOCX: `all` (default) keeps insertions underlined and deletions struck through; `final` shows the revised text and `original` shows the text before those changes. This does not accept or reject revisions in the source file. |
...
- CAD export captures the current camera view / current native DWF page as a PNG, including WebGL vectors and text overlays. Source mode uses a dark background and adaptive contrast; monochrome uses black lines on white paper. Explicit background options take priority. Use Fit before exporting a whole drawing; this is not a multi-sheet vector PDF converter or a separate PNG/JPEG download command.
